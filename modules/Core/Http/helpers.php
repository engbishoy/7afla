                    ...
                    try {
                        ...
                        if(config()->has($module->getLowerName(). '.menus.'. $menu_key. '.' . $resourceKey . '.sub_menu')){
                            ...
                            foreach (config($module->getLowerName(). '.menus.' .$menu_key. '.' . $resourceKey . '.sub_menu') as $key => $value) {
                                $item = [];
                                $item['title_en'] = config($module->getLowerName(). '.menus.'. $menu_key . '.' . $resourceKey . '.sub_menu.' . $key . '.title_en');
                                $item['title_ar'] = config($module->getLowerName(). '.menus.'. $menu_key . '.' . $resourceKey . '.sub_menu.' . $key . '.title_ar');
